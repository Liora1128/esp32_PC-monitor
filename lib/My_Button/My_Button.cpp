...
static const gpio_num_t kPins[MY_BUTTON_NUM] = {
    GPIO_NUM_1, GPIO_NUM_2, GPIO_NUM_42,
};
...
static void debounce_cb(void *arg)
{
    ...
    if (idx < 0 || idx >= MY_BUTTON_NUM) return;
    Button_t *b = &s_btns[idx];
    if (gpio_get_level(b->pin) != 0) return;     // 抖动丢弃
    b->state = 1;
    b->long_fired_at = 0;
    b->hold_fired_at = 0;
...
static void long_cb(void *arg)
{
    ...
    if (idx < 0 || idx >= MY_BUTTON_NUM) return;
    Button_t *b = &s_btns[idx];
    if (gpio_get_level(b->pin) != 0) return;      // 早已松开
    if (b->long_fired_at != 0) return;           // 已报过
...
static void hold_cb(void *arg)
{
    ...
    Button_t *b = &s_btns[idx];
    if (gpio_get_level(b->pin) != 0) return;      // 早已松开
    if (b->hold_fired_at != 0) return;           // 已报过
...
{
    if (!s_evt_queue) return KEY_NONE;
